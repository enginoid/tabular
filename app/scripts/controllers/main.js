...
angular.module('angularTableApp', []).
  ...
  controller('MainCtrl', ['$scope', 'localeCompareFunctions', function ($scope, localeCompareFunctions) {
    var getLocaleCompareFunc = function (columnName, locale) {
      return function (o1, o2) {
        ...
        var localeCompareFunction = localeCompareFunctions[locale];
        if (localeCompareFunction) {
          return localeCompareFunction(v1, v2);
        } else {
    ...
    $scope.columns = [
      {name: 'Name', id: 'name', sortPredicate: getLocaleCompareFunc('name', 'is')},
